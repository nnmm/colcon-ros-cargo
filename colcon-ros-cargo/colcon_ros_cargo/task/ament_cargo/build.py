...
class AmentCargoBuildTask(CargoBuildTask):
    ...
    def add_arguments(self, *, parser):  # noqa: D102
        parser.add_argument(
            '--lookup-only-install',
            action='store_true',
            help='Look up dependencies in the installation prefixes only. '
            'By default, dependencies are looked up in the workspace whenever '
            "possible, which does not strictly follow colcon's intended "
            'semantics.')
    ...
    async def _build(self, args, env):
        ...
        if package_paths is None:
            if args.lookup_only_install:
                package_paths = {}
            else:
                package_paths = find_workspace_cargo_packages(args.install_base)  # noqa: E501

        # Any message Rust packages that were generated after the build process
        # started are not yet in package_paths. So we'll need to scan the
        # install dirs, aka prefixes.
